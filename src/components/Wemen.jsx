...
const Women = ({ onAddToCart, searchQuery }) => {
  ...
  // Fetch women products with server-side filtering
  const fetchWomenProducts = useCallback(async (pageNum = 1) => {
    if (pageNum === 1) {
    ...
    try {
      ...
      // Add category filter
      q = query(q, where("category", "==", "women"));
  ...
  const loadMore = useCallback(() => {
    if (!loading && hasMore) {
      fetchWomenProducts(page + 1);
    }
  }, [loading, hasMore, page, fetchWomenProducts]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          <SectionHeader title="Women's Collection" count={products.length} />
          <p className="text-gray-600 text-sm">
            Browse our premium selection of women's products
          </p>
        </div>
